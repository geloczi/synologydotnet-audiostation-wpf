...
namespace MusicPlayback.Utils
{
    public class MyBufferedWaveProvider : IWaveProvider
    {
        ...
        /// <param name="bufferLengthInSeconds">Length of the buffer in seconds</param>
        public MyBufferedWaveProvider(WaveFormat waveFormat, int bufferLengthInSeconds)
        {
            if (bufferLengthInSeconds < 1)
                throw new ArgumentOutOfRangeException(nameof(bufferLengthInSeconds));

            this.waveFormat = waveFormat;
            BufferLength = waveFormat.AverageBytesPerSecond * bufferLengthInSeconds;
            ReadFully = true;
        }
